...
class CreditService:
    ...
    async def get_balance(self, organization_id: str) -> Dict[str, Any]:
        ...
        try:
            result = self.supabase.table("credit_balances").select("*").eq(
            ...
            if not result.data:
                ...
                result = self.supabase.table("credit_balances").insert(balance_data).execute()
    ...
    async def consume_credits(
        ...
        try:
            ...
            if balance["available"] < 10:
                await self._emit_low_balance_event(organization_id, balance["available"])
    ...
    async def get_usage_history(
        ...
        try:
            query = self.supabase.table("credit_usage").select("*").eq(
                "organization_id", organization_id
            )
    ...
    async def get_transaction_history(
        ...
        try:
            query = self.supabase.table("credit_transactions").select("*").eq(
                "organization_id", organization_id
            )
    ...
    async def get_agent_action_costs(self) -> List[Dict[str, Any]]:
        ...
        try:
            result = self.supabase.table("agent_action_costs").select("*").eq(
                "is_active", True
            ).execute()
    ...
    async def get_cost_for_action(self, agent_type: str, action: str) -> float:
        ...
        try:
            result = self.supabase.table("agent_action_costs").select("credit_cost").eq(
                "agent_type", agent_type
            ).eq("action", action).eq("is_active", True).execute()
            
            if result.data:
                return float(result.data[0]["credit_cost"])
            
            # Default cost if not configured
            logger.warning(f"No cost configured for {agent_type}.{action}, using default")
            return 1.0
    ...
    async def _check_department_limits(self, department_id: str, amount: float):
        ...
        try:
            ...
            limits_result = self.supabase.table("department_credit_limits").select("*").eq(
            ...
            if limits.get("daily_limit"):
                ...
                usage_result = self.supabase.table("credit_usage").select(
                    "credits_consumed"
                ).eq("department_id", department_id).gte(
                    "created_at", today_start.isoformat()
                ).execute()
                
                today_usage = sum(float(u["credits_consumed"]) for u in usage_result.data)
            ...
            if limits.get("monthly_limit"):
                ...
                month_usage = sum(float(u["credits_consumed"]) for u in usage_result.data)
    ...
    async def set_department_limits(
        ...
        try:
            ...
            existing = self.supabase.table("department_credit_limits").select("id").eq(
                "department_id", department_id
            ).execute()
            ...
            if existing.data:
                # Update existing
                self.supabase.table("department_credit_limits").update(
                    limit_data
